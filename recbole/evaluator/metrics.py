...
class AveragePrice(AbstractMetric):
    r"""AveragePrice computes the average price of recommended items.

    .. math::
        \mathrm{AveragePrice@K}=\frac{1}{|U|} \sum_{u \in U } \frac{\sum_{i \in R_{u}} price(i)}{|R_{u}|}

    :math:`price(i)` is the price of item i.
    """

    metric_type = EvaluatorType.RANKING
    metric_need = ["rec.items", "data.item_price"]
    ...
    def used_info(self, dataobject):
        """Get the matrix of recommendation items and the price of items"""
        item_price = dataobject.get("data.item_price")
        item_matrix = dataobject.get("rec.items")
        # print(f"item_price: {item_price[:5]}")  # 打印前5个物品价格
        # print(f"item_matrix: {item_matrix[:5]}")  # 打印前5行推荐物品矩阵
        return item_matrix.numpy(), item_price  # 直接返回 item_price

    def calculate_metric(self, dataobject):
        item_matrix, item_price = self.used_info(dataobject)
        result = self.metric_info(self.get_prices(item_matrix, item_price))
        metric_dict = self.topk_result("averageprice", result)
        return metric_dict

    def get_prices(self, item_matrix, item_price):
        """Convert the matrix of item id to the matrix of item price using a dict:{id,price}."""
        value = np.zeros_like(item_matrix, dtype=float)
        for i in range(item_matrix.shape[0]):
            row = item_matrix[i, :]
            for j in range(row.shape[0]):
                value[i][j] = item_price[row[j]].item()  # 确保 item_price 是一个张量或数组
        print("Max price value:", value.max())
        return value
...
class AverageTime(AbstractMetric):
    r"""AverageTime computes the average time of recommended items.

    .. math::
        \mathrm{AverageTime@K}=\frac{1}{|U|} \sum_{u \in U } \frac{\sum_{i \in R_{u}} time(i)}{|R_{u}|}

    :math:`time(i)` is the time of item i.
    """

    metric_type = EvaluatorType.RANKING
    metric_need = ["rec.items", "data.item_time"]
    ...
    def used_info(self, dataobject):
        """Get the matrix of recommendation items and the time of items"""
        item_time = dataobject.get("data.item_time")
        item_matrix = dataobject.get("rec.items")
        return item_matrix.numpy(), item_time  # 直接返回 item_time

    def calculate_metric(self, dataobject):
        item_matrix, item_time = self.used_info(dataobject)
        result = self.metric_info(self.get_times(item_matrix, item_time))
        metric_dict = self.topk_result("averagetime", result)
        return metric_dict

    def get_times(self, item_matrix, item_time):
        """Convert the matrix of item id to the matrix of item time using a dict:{id,time}."""
        value = np.zeros_like(item_matrix, dtype=float)
        for i in range(item_matrix.shape[0]):
            row = item_matrix[i, :]
            for j in range(row.shape[0]):
                value[i][j] = item_time[row[j]].item()  # 确保 item_time 是一个张量或数组
        print("Max time value:", value.max())
        return value
