...
class Trainer(AbstractTrainer):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                if update_flag:
                    ...
                    if hasattr(self.model, 'save_best_epoch_embeddings'):
                        self.model.save_best_epoch_embeddings()
    ...
    @torch.no_grad()
    def evaluate(
        self, eval_data, load_best_model=True, model_file=None, show_progress=False,final=False
    ):
        r"""Evaluate the model based on the eval data.
        ...
        if not eval_data:
            return
        self.eval_collector.data_collect(eval_data)
        if load_best_model:
        ...
        # 在评估结束后调用 analyze_feature_importance
        if final:
            if hasattr(self.model, 'analyze_feature_importance'):
                importance_results = self.analyze_feature_importance(eval_data)
                if importance_results:
                    for feature, importance in importance_results.items():
                        self.logger.info(f"{feature}: {importance}")
            if hasattr(self.model, 'get_original_user_id') and hasattr(self.model, 'get_original_item_id'):
                self.save_recommendations(struct,eval_data)

        return result

    def save_recommendations(self, struct,eval_data):
        self.logger.info("Struct content:")
        if not hasattr(struct, '_data_dict'):
            self.logger.error("struct does not have _data_dict attribute")
            return

        topk = self.config['topk'][0]
        recommendations = {}
        # 尝试从 _data_dict 中获取推荐项目
        if 'rec.items' in struct._data_dict:
            rec_items = struct._data_dict['rec.items']
            
        elif 'rec.topk' in struct._data_dict:
            rec_items = struct._data_dict['rec.topk']
            
        else:
            self.logger.error("Cannot find recommendation items in struct._data_dict")
            return
        
        user_ids = eval_data.dataset.get_user_feature()[self.config['USER_ID_FIELD']]

        if isinstance(rec_items, torch.Tensor):
            for idx, (user_id, user_items) in enumerate(zip(user_ids, rec_items)):
                original_user_id = self.model.get_original_user_id(user_id)
                original_item_ids = [self.model.get_original_item_id(torch.tensor(item_idx.item())) for item_idx in user_items[:topk]]
                recommendations[original_user_id] = original_item_ids
        else:
            self.logger.error(f"Unexpected type for rec_items: {type(rec_items)}")
            return

        # 创建保存推荐结果的文件夹
        folder_name = f"{self.config['model']}_recommendation_result"
        if not os.path.exists(folder_name):
            os.makedirs(folder_name)

        # 在文件夹中保存推荐结果
        file_path = os.path.join(folder_name, 'recommendations.txt')
        with open(file_path, 'w') as f:
            for user_id, item_ids in recommendations.items():
                f.write(f"User {user_id}: {','.join(map(str, item_ids))}\n")

        self.logger.info(f"Recommendations saved to {file_path}")
