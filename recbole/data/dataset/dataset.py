...
class Dataset(torch.utils.data.Dataset):
    ...
    def _data_processing(self):
        ...
        self._set_item_price()
        self._set_item_time()

    def _set_item_price(self):
        if self.item_feat is not None and self.config["ITEM_PRICE_FIELD"] in self.item_feat:
            # 将 Pandas Series 转换为 PyTorch 张量
            self.item_price = torch.tensor(self.item_feat[self.config["ITEM_PRICE_FIELD"]].values.round(2), dtype=torch.float32)
            # print(f"self.item_feat.head(){self.item_feat.head()},{len(self.item_feat)}")  # 检查原始数据的前几行
            # print(f"self.item_feat['item_price'].values{self.item_feat['item_price'].values}")
            # 然后将张量移动到指定的设备
            self.item_price = self.item_price.to(self.device)
            self.logger.info(f"Item price set: {self.item_price[:5]}...")  # 打印前5个价格
        else:
            self.logger.warning('Item price not found in dataset, using default values.')
            self.item_price = torch.ones(self.item_num, dtype=torch.float32).to(self.device)
        # print(f"_set_item_price called, item_price: {self.item_price[:5]}")  # 添加日志

    def _set_item_time(self):
        if self.item_feat is not None and self.config["ITEM_TIME_FIELD"] in self.item_feat:
            # 将 Pandas Series 转换为 PyTorch 张量
            self.item_time = torch.tensor(self.item_feat[self.config["ITEM_TIME_FIELD"]].values.round(2), dtype=torch.float32)
            self.item_time = self.item_time.to(self.device)
            print(f"Item time set: {self.item_feat['response_time'].values}...")  # 打印前5个时间
        else:
            self.logger.warning('Item time not found in dataset, using default values.')
            self.item_time = torch.ones(self.item_num, dtype=torch.float32).to(self.device)
        self.logger.info(f"Item time set: ...")  # 打印前5个时间
    ...
    def _build_feat_name_list(self):
        ...
        print(f"feat_name_list{feat_name_list}")
    ...
    def _load_inter_feat(self, token, dataset_path):
        ...
        if self.benchmark_filename_list is None:
            ...
            print(f"Loading data from: {inter_feat_path}")
            print(f"File exists: {os.path.exists(inter_feat_path)}")
            ...
            print("_load_inter_feat-else")
        print(f"_load_inter_feat={self.inter_feat}")
    ...
    def _load_feat(self, filepath, source):
        ...
        if load_col == set():
            return None
        print(f"load_col={load_col}, unload_col={unload_col}")
        ...
        for field_type in head.split(field_separator):
            ...
            print(f"field_separator{field}, {ftype}")
            try:
        ...
        for field in columns:
            ...
            print(f"field={field},ftype={ftype}")
            if not ftype.value.endswith("seq"):
                continue
    ...
    def _user_item_feat_preparation(self):
        ...
        if self.item_feat is not None:
            ...
            print(f"_user_item_feat_preparation,{self.item_feat.head()},{len(self.item_feat)}")
    ...
    def _fill_nan(self):
        ...
        for feat_name in self.feat_name_list:
            ...
            for field in feat:
                if field in self.field2type:
                    ftype = self.field2type[field]
                    if ftype == FeatureType.TOKEN:
                        if pd.api.types.is_numeric_dtype(feat[field]):
    ...
    def _concat_remaped_tokens(self, remap_list):
        ...
        for feat, field, ftype in remap_list:
            if ftype == FeatureType.TOKEN:
                ...
                print("deal.TOKEN_SEQ")
                feat[field] = feat[field].apply(lambda x: x if isinstance(x, list) else ([] if pd.isna(x).all() else x))
                try:
                    tokens.append(feat[field].agg(np.concatenate))
                except KeyError as e:
                    print(f"KeyError encountered for field: {field}, error: {e}")
        split_point = np.cumsum(list(map(len, tokens)))[:-1]
        if tokens:
            tokens = np.concatenate(tokens)
        else:
            tokens = np.array([])  # 如果为空，返回空数组
        ...
        for (feat, field, ftype), new_ids in zip(remap_list, new_ids_list):
            ...
            if ftype == FeatureType.TOKEN:
                if field == self.uid_field:
                    self.user_id_map = dict(zip(new_ids, feat[field]))
                elif field == self.iid_field:
                    self.item_id_map = dict(zip(new_ids, feat[field]))
                feat[field] = new_ids
            elif ftype == FeatureType.TOKEN_SEQ:
                split_point = np.cumsum(feat[field].agg(len))[:-1]
    ...
    def copy_field_property(self, dest_field, source_field):
        ...
        """
        if source_field in self.field2type:
            self.field2type[dest_field] = self.field2type[source_field]
        else:
            self.logger.warning(f"Source field '{source_field}' not found in field2source. Skipping copy for '{dest_field}'.")
            self.field2type[dest_field] = None 
        if source_field in self.field2source:
            self.field2source[dest_field] = self.field2source[source_field]
        else:
            self.logger.warning(f"Source field '{source_field}' not found in field2source. Skipping copy for '{dest_field}'.")
            self.field2source[dest_field] = None 
        if source_field in self.field2seqlen:
            self.field2seqlen[dest_field] = self.field2seqlen[source_field]
        else:
            self.logger.warning(f"Source field '{source_field}' not found in field2source. Skipping copy for '{dest_field}'.")
            self.field2seqlen[dest_field] = None 
    ...
    def get_inter_feature(self):
        """
        Returns:
            Interaction: item features
        """
        if self.inter_feat is None:
            return None
        else:
            print(f"get_item_feature{self.item_feat}")
            return self.inter_feat
